...
import { gameIdentity, getGameEcosystem, libraryMatchesDeal } from '../shared/gameIdentity'
import { exportLibraryBackup, IMPORT_MAX_BYTES, parseLibraryImport, type ImportPreview } from '../lib/importLibrary'
...
export function LibraryPanel({ state, language, ecosystem = 'pc', country, deals, busy, error, onAction, onCheck, onOpenGame }: LibraryPanelProps) {
  ...
  const [formEcosystem, setFormEcosystem] = useState<GameEcosystem | undefined>()
  const [platformFilter, setPlatformFilter] = useState<GameEcosystem | 'all' | 'current'>('current')
  const addEcosystem = formEcosystem ?? ecosystem
  const selectedPlatform = platformFilter === 'current' ? ecosystem : platformFilter
  const platformName = (value: GameEcosystem) => value === 'playstation' ? 'PlayStation' : value === 'xbox' ? 'Xbox' : 'PC'
  ...
  const filtered = useMemo(() => state.games.filter((game) => (selectedPlatform === 'all' || getGameEcosystem(game) === selectedPlatform) && (filter === 'all' || (filter === 'watched' ? game.watched : game.owned)) && game.title.toLowerCase().includes(query.trim().toLowerCase())).sort((a, b) => a.priority - b.priority || a.title.localeCompare(b.title)), [state.games, filter, query, selectedPlatform])
  ...
  const add = async (event: React.FormEvent) => {
    ...
    const normalizedProductId = addEcosystem === 'pc' ? undefined : productId.trim().toUpperCase() || undefined
    const id = gameIdentity({ title, ecosystem: addEcosystem, storeProductId: normalizedProductId })
    const validId = !normalizedProductId || (addEcosystem === 'playstation' ? /^[A-Z]{2}\d{4}-[A-Z0-9]{9}_\d{2}-[A-Z0-9]{16}$/.test(normalizedProductId) : /^[A-Z0-9]{12}$/.test(normalizedProductId))
    if (!title.trim() || !id || !validId || (addEcosystem === 'pc' && steamId && !/^\d{1,12}$/.test(steamId))) { setLocalError(es ? 'Revisa el título y el ID opcional de la tienda seleccionada.' : 'Check the title and the optional ID for the selected store.'); return }
    const existing = state.games.find((game) => game.id === id)
    try {
      await onAction({ action: 'upsert', game: { ...existing, id, title: title.trim(), ecosystem: addEcosystem, storeProductId: normalizedProductId, steamAppId: addEcosystem === 'pc' ? steamId || existing?.steamAppId : undefined, owned: existing?.owned ?? false, watched: true, priority: existing?.priority ?? 2, notes: existing?.notes ?? '', updatedAt: new Date().toISOString() } })
      setTitle(''); setSteamId(''); setProductId(''); setAdding(false); setPlatformFilter(addEcosystem); setStatus(es ? 'Juego añadido a tus favoritos.' : 'Game added to favorites.')
  ...
  return <section className="personal-panel" aria-label={es ? 'Asistente personal de ofertas' : 'Personal deals assistant'} aria-busy={busy || reading}>
    ...
    {view === 'games' ? <>
      ...
      <details className="personal-import-help"><summary>{es ? 'Formato de importación y privacidad' : 'Import format and privacy'}</summary><p>{es ? 'Archivos locales CSV o JSON, máximo 2 MB y 500 juegos. El archivo se lee aquí; solo los registros validados se envían a la API local de DealRift. No necesitas contraseña, cookies ni claves de Steam, PlayStation o Xbox.' : 'Local CSV or JSON files, up to 2 MB and 500 games. The file is read here; only validated records are sent to DealRift’s local API. No Steam, PlayStation or Xbox password, cookies or API key needed.'}</p><p>{es ? 'CSV: title obligatorio; ecosystem (pc, playstation, xbox), storeProductId, steamAppId, owned, watched, priority, notes, targetAmount y targetCurrency opcionales. JSON: array de juegos o copia exportada. owned/watched usan true/false; prioridad 1=alta, 2=normal, 3=baja.' : 'CSV: title required; ecosystem (pc, playstation, xbox), storeProductId, steamAppId, owned, watched, priority, notes, targetAmount and targetCurrency optional. JSON: game array or exported backup. owned/watched use true/false; priority 1=high, 2=normal, 3=low.'}</p><code>title,steamAppId,owned,watched,priority,targetAmount,targetCurrency<br />Portal 2,620,false,true,1,5,USD</code></details>
      {adding ? <form className="personal-section" onSubmit={(event) => void add(event)}><h3>{es ? 'Añadir un favorito' : 'Add a favorite'}</h3><div className="personal-form-row"><label>{es ? 'Título completo (obligatorio)' : 'Full title (required)'}<input ref={titleInput} type="text" autoComplete="off" maxLength={300} required value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>{es ? 'Plataforma' : 'Platform'}<select value={addEcosystem} onChange={(event) => { setFormEcosystem(event.target.value as GameEcosystem); setProductId(''); setSteamId('') }}><option value="pc">PC</option><option value="playstation">PlayStation</option><option value="xbox">Xbox</option></select></label>
        {addEcosystem === 'pc' ? <label>{es ? 'ID de Steam (opcional)' : 'Steam app ID (optional)'}<input type="text" inputMode="numeric" autoComplete="off" pattern="[0-9]{1,12}" maxLength={12} value={steamId} onChange={(event) => setSteamId(event.target.value)} /></label> : <label>{es ? 'ID del producto (opcional)' : 'Product ID (optional)'}<input type="text" autoComplete="off" maxLength={48} value={productId} onChange={(event) => setProductId(event.target.value)} placeholder={addEcosystem === 'playstation' ? 'UP4497-PPSA03972_00-00000000000GOTY7' : '9MZ0SR207MG8'} /></label>}<button type="submit" className="personal-primary" disabled={busy}>{es ? 'Guardar favorito' : 'Save favorite'}</button><button type="button" onClick={() => setAdding(false)}>{es ? 'Cancelar' : 'Cancel'}</button></div><p className="personal-fine-print">{addEcosystem === 'pc' ? (es ? 'Conserva el nombre exacto de la edición. El ID opcional es el número en la URL de Steam /app/620/; no identifica por sí solo una edición.' : 'Keep the exact edition name. The optional ID is the number in a Steam URL such as /app/620/; it does not identify an edition on its own.') : (es ? 'Conserva el título exacto y copia el ID al final de la URL de la tienda. Guardar una oferta del radar conserva automáticamente su edición y consolas compatibles. Poseerlo en otra plataforma no cuenta como poseer esta licencia.' : 'Keep the exact title and copy the ID at the end of the store URL. Saving a radar offer automatically preserves its edition and compatible consoles. Ownership on another platform does not count as this license.')}</p></form> : null}
      ...
      <div className="personal-form-row"><label>{es ? 'Plataformas de tu biblioteca' : 'Library platforms'}<select value={platformFilter} onChange={(event) => setPlatformFilter(event.target.value as typeof platformFilter)}><option value="current">{es ? 'Como el radar' : 'Same as radar'} · {platformName(ecosystem)}</option><option value="all">{es ? 'Todas las plataformas' : 'All platforms'}</option><option value="pc">PC</option><option value="playstation">PlayStation</option><option value="xbox">Xbox</option></select></label></div>
